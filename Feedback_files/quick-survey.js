...
eesy.define('quick-survey', ['jquery-private', 'utils', 'helpitem-loader', 'json!settings-supportcenter'], function ($, utils, helpItemLoader, settings) {
    ...
    function handleSubmitVoteSupport(element, isUpVote) {
        var reason = $('.layout__main ' +
            (isUpVote ? '.__explain-upvoting' : '.__explain-downvoting') +
            ' .quick-survey__textarea').val();
    ...
    function sendVoteSupport(reason, element, isUp) {
        ...
        var voteOptionsById = {};
        $(element)
            .closest('.layout__main')
            .find('#votingUpCheckBoxes input:checkbox:checked')
            .each(function () {
            voteOptionsById[$(this).data('id')] = $(this).data('text');
        });
        $(element)
            .closest('.layout__main')
            .find('#votingDownCheckBoxes input:checkbox:checked')
            .each(function () {
            voteOptionsById[$(this).data('id')] = $(this).data('text');
        });
        var appearAnonymous = $(element)
            .closest('.layout__main')
            .find('.eesy_anonymous_switch input')
            .is(':checked');
        var data = {
            sessionKey: var_key,
            isUp: isUp,
        ...
        if (settings.SUPPORTCENTER.VOTINGOPTIONS.ENABLED) {
            $.getJSON(var_dashboard_url + '/rest/helpitems/options/voting?type=' + direction + '&sessionkey=' + var_key, function (votingoptions) {
                ...
                votingoptions.forEach(function (votingOption) {
                    var checkboxElement = '<input type="checkbox"  aria-label="' +
                        votingOption.caption +
                        '" data-id="' +
                        votingOption.id +
                        '" data-text="' +
                        votingOption.caption +
                ...
                if (votingoptions.length === 0) {
                    $(button)
                        .closest('.layout__main')
                        .find('#votingDownCheckBoxesHeader')
                        .addClass('hideQuestion');
                    $(button)
                        .closest('.layout__main')
                        .find('#votingUpCheckBoxesHeader')
                        .addClass('hideQuestion');
                }
                else {
    ...
    function sendVoteMessage(reason, element, isUp) {
        ...
        $(element)
            .closest('.quick-survey-section')
            .find('#votingUpCheckBoxes input:checked')
            .each(function () {
            voteOptionsById[$(this).data('id')] = $(this).data('text');
        });
        $(element)
            .closest('.quick-survey-section')
            .find('#votingDownCheckBoxes input:checked')
            .each(function () {
            voteOptionsById[$(this).data('id')] = $(this).data('text');
        });
        var appearAnonymous = $(element)
    ...
    utils.onClickOrSelectKey('.quick-survey__answer', function (e, button) {
        ...
        $(button)
            .closest('.quick-survey-section')
            .children(answer ? '.__explain-upvoting' : '.__explain-downvoting')
            .removeClass('hideQuestion');
        $(button)
            .closest('.quick-survey-section')
            .children(!answer ? '.__explain-upvoting' : '.__explain-downvoting')
            .addClass('hideQuestion');
        ...
        $(button)
            .closest('.quick-survey-section')
            .find('.__explain-upvoting')
            .find('.quick-survey__textarea')
            .val('');
        $(button)
            .closest('.quick-survey-section')
            .find('.__explain-downvoting')
            .find('.quick-survey__textarea')
            .val('');
            ...
            $.getJSON(var_dashboard_url + '/rest/helpitems/options/voting?type=' + direction + '&sessionkey=' + var_key, function (votingoptions) {
                ...
                votingoptions.forEach(function (votingOption) {
                    var checkboxElement = '' +
                        ...
                        '<span class="eesy_hide_text_voting">' +
                        votingOption.caption +
                        '</span>\n' +
                        '</label>';
                    var appendToElement = direction === 'up' ? '#votingUpCheckBoxes' : '#votingDownCheckBoxes';
