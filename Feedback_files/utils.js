eesy.define(['jquery-private'], function ($) {
    ...
    function isOrHas(element, selector) {
        return !!element.getAttribute && (!!$(element).parents(selector).length || $(element).is(selector));
    }
    ...
    function isTargetVisible(target, connectTo) {
        ...
        return (
            isSameOrChildOf(connectTo, elementFromAdjustedPoint(left, top)) ||
    ...
    function getFocusableElements(element) {
        const selectors = 'a[href], button, input, textarea, select, details, iframe, [tabindex]:not([tabindex="-1"])';
        const filter = (el) =>
            !el.hasAttribute('disabled') && !el.getAttribute('aria-hidden') && el.offsetParent !== null;
        return Array.from(element.querySelectorAll(selectors)).filter(filter);
    }

    function getFirstFocusableElement(element) {
        return getFocusableElements(element)[0] ?? null;
    }

    function getNextFocusableElement(focusableElement, container) {
        const focusableElements = getFocusableElements(container);
        return focusableElements[focusableElements.indexOf(focusableElement) + 1];
    }
    ...
    function isFocusVisible(element) {
        try {
            ...
            // fallback for older browsers that does not support :focus-visible selector
            return false;
        }
    }
